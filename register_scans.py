...
def check_positive(value):
    ...
    if ivalue <= 0:
        raise argparse.ArgumentTypeError(
            "%s has to be a positive int value > 0" % value
        )
    return ivalue


def load_pointcloud(cloud_id, args=None, preview=True):
    ...
    print(f"Finished loading point cloud from file {file} ...")
    ...
    pointcloud, ids = pointcloud.remove_radius_outlier(
        args.filter_nb_points, args.filter_radius
    )
...
def pick_points(pcd, node_from, node_to, node_current):
    ...
    print(
        "| If you messed something up in the first point cloud of the current pair  |"
    )
    print(
        "| of point clouds, simply select no points and press q. By doing this the  |"
    )
    print(
        "| procedure will be restarted for the current pair of point clouds.        |"
    )
    print(
        "+--------------------------------------------------------------------------+"
    )
    ...
    vis.create_window(
        f"Pick correspondences between point cloud {node_from} and {node_to} - point cloud {node_current}"
    )
...
def register_pointcloud_pair(node_from, node_to, pointclouds, args):
    ...
    if os.path.isfile(transformation_file) and os.path.isfile(information_file):
        print(
            f"{color.BOLD}Found registration of {node_from} and {node_to}{color.ENDC}"
        )
        ...
        if not args.refine_all and node_to not in args.refine_list:
            ...
            print(
                f"{color.BOLD}Refining registration of {node_from} and {node_to}{color.ENDC}"
            )
    else:
        ...
        picked_points_to = []
        while (
            len(picked_points_from) < 3
            or len(picked_points_to) < 3
            or len(picked_points_from) != len(picked_points_to)
        ):
            picked_points_from = pick_points(
                get_cloud(pointclouds, args, node_from), node_from, node_to, node_from
            )
            picked_points_to = pick_points(
                get_cloud(pointclouds, args, node_to), node_from, node_to, node_to
            )
        ...
        print(
            f"{color.BOLD}Compute a rough transform using the correspondences given by user ...{color.ENDC}"
        )
    ...
    if show_result:
        ...
        def bad_reg(vis):
            ...
            bad = True
            print(
                f"{color.WARNING}Registration has been marked as BAD. Press q to retry!{color.ENDC}"
            )
        ...
        print(
            "| Take a look at the combined point cloud. If the registration looks bad,  |"
        )
        print(
            "| press B and then q to rerun the registration of the current point clouds.|"
        )
        print(
            "| If everything looks fine, just press q.                                  |"
        )
        print(
            "+--------------------------------------------------------------------------+"
        )
        ...
        if bad:
            print(
                f"{color.WARNING}Retrying registration of point cloud {node_from} and {node_to}.{color.ENDC}"
            )
...
def register_pointclouds(pointclouds, nx_pose_graph, args):
    ...
    for node_from, node_to in nx.edge_dfs(nx_pose_graph):
        ...
        print(
            f"{color.BOLD}Starting registration between point cloud {node_from} and {node_to} ...{color.ENDC}"
        )
        transformation, icp_information = register_pointcloud_pair(
            node_from, node_to, pointclouds, args
        )
        ...
        if (o3d_pose_graph.nodes[node_to_id].pose == np.identity(4)).all():
            o3d_pose_graph.nodes[node_to_id].pose = np.dot(
                o3d_pose_graph.nodes[node_from_id].pose, transformation
            )

        # add edge to pose graph
        # if we have a loop closure and didnt update the pose, we mark our edge as uncertain
        print(
            f"{color.BOLD}Adding new edge from point cloud {node_from} (id {node_from_id}) to point cloud {node_to} (id {node_to_id}){color.ENDC}"
        )
        o3d_pose_graph.edges.append(
            o3d.pipelines.registration.PoseGraphEdge(
                ...
                icp_information,
                uncertain=(
                    o3d_pose_graph.nodes[node_to_id].pose != np.identity(4)
                ).all(),
            )
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Register several scans using Open3D",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        action="store",
        help="Dataset directory, point cloud input directory",
        type=str,
    )
    ...
    parser.add_argument(
        ...
        help='directory of the temporary files that store the current progress of the registration. \
                            (inside your dataset directory) default: "transformation"',
        type=str,
    )
    ...
    parser.add_argument(
        ...
        action="store",
        required=False,
        default=scan_file_pattern,
        type=str,
        help="scan file pattern with file extension. default: {}".format(
            scan_file_pattern
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store",
        required=False,
        default=trans_file_pattern,
        type=str,
        help="transformation file pattern with file extension, default: {}".format(
            trans_file_pattern
        ),
    )
    ...
    parser.add_argument(
        ...
        default=3,
        type=int,
        help="Fill up id with zeros to have at least this amount of digits",
    )
    ...
    parser.add_argument(
        ...
        action="store",
        nargs="+",
        help='list of nodes whose poses should be refined (this does only work if "--no-icp" is not set)',
        type=str,
    )
    ...
    print(
        f"{color.OKBLUE}{color.BOLD}Finished loading graph and point clouds{color.ENDC}"
    )

    # check pose graph correctness
    if len(list(nx.simple_cycles(nx_pose_graph))) > 0:
        print(
            f"{color.FAIL}ERROR: There are no circles allowed in the pose graph!{color.ENDC}"
        )
    ...
    root_nodes = [
        node
        for node in nx_pose_graph.nodes()
        if len(list(nx_pose_graph.in_edges(node))) == 0
    ]
    if len(root_nodes) != 1:
        print(
            f"{color.FAIL}ERROR: There is only one node with in-degree 1 (origin node) allowed in the pose graph!{color.ENDC}"
        )
        return
    ...
    o3d_pose_graph, node_id_mapping = register_pointclouds(
        pointclouds, nx_pose_graph, args
    )
    ...
    for node in node_id_mapping.keys():
        ...
        np.savetxt(
            args.trans_pattern.replace("*", node_text),
            o3d_pose_graph.nodes[node_id].pose,
        )
